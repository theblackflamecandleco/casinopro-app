...
// !!! PASTE YOUR FIREBASE CONFIG FROM THE GOOGLE CONSOLE HERE !!!
...
export default function App() {
  ...
  const approveReview = async (rev) => {
    if (!isAdmin) return;
    await updateDoc(doc(db, 'casinos', rev.casinoId, 'user_reviews', rev.id), { isApproved: true });
  };

  const deleteReview = async (rev) => {
    if (!isAdmin) return;
    await deleteDoc(doc(db, 'casinos', rev.casinoId, 'user_reviews', rev.id));
  };
  ...
  const ListView = () => (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="text-center py-10">
        ...
        <p className="text-slate-400">Expert reviews and exclusive bonuses.</p>
      </div>
      <div className="grid gap-6">
        {casinos.map((c, i) => (
          <div key={c.id} onClick={() => { setSelectedCasino(c); setView('details'); }} className="cursor-pointer bg-slate-900/60 border border-slate-800 rounded-[2rem] p-6 flex flex-col md:flex-row items-center gap-6 hover:border-indigo-500/50 transition-all">
            <div className="w-24 h-24 rounded-2xl flex items-center justify-center text-3xl font-black shadow-lg" style={{ backgroundColor: c.color }}>{c.name.charAt(0)}</div>
  ...
  const DetailsView = () => (
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in duration-300">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-[3rem] overflow-hidden">
        ...
        <div className="px-10 pb-10 -mt-16">
          <div className="flex flex-col md:flex-row items-end gap-6 mb-8">
            <div className="w-32 h-32 rounded-3xl flex items-center justify-center text-5xl font-black border-4 border-slate-900" style={{ backgroundColor: selectedCasino.color }}>{selectedCasino.name.charAt(0)}</div>
            ...
            <a href={selectedCasino.link} target="_blank" className="w-full md:w-auto bg-indigo-600 px-10 py-4 rounded-2xl font-black text-center shadow-lg shadow-indigo-600/20">PLAY NOW</a>
          </div>
      ...
      <div className="space-y-8">
        <h2 className="text-2xl font-black flex items-center gap-3"><MessageSquare className="text-indigo-500" /> Community Feedback</h2>
        <div className="bg-slate-900/50 border border-slate-800 p-8 rounded-[2rem]">
          <form onSubmit={submitReview} className="space-y-4">
            ...
            <button type="submit" disabled={(!user || user.isAnonymous)} className="bg-indigo-600 px-8 py-3 rounded-xl font-black text-sm flex items-center gap-2">SUBMIT REVIEW</button>
          </form>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-8 rounded-[2rem]">
            <form onSubmit={handleSaveCasino} className="grid md:grid-cols-2 gap-6">
              <input className="bg-slate-800 rounded-xl p-4" placeholder="Name" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} required />
              <select className="bg-slate-800 rounded-xl p-4" value={formData.rating} onChange={e => setFormData({...formData, rating: e.target.value})}><option value="5">5 Stars</option><option value="4.5">4.5 Stars</option></select>
              <input className="md:col-span-2 bg-slate-800 rounded-xl p-4" placeholder="Bonus" value={formData.bonus} onChange={e => setFormData({...formData, bonus: e.target.value})} required />
              ...
              <textarea className="md:col-span-2 bg-slate-800 rounded-xl p-4 h-32" placeholder="Description" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
              <input className="bg-slate-800 rounded-xl p-4" placeholder="Features" value={formData.features} onChange={e => setFormData({...formData, features: e.target.value})} />
              ...
              <button className="md:col-span-2 bg-indigo-600 py-4 rounded-2xl font-black">{isEditing ? 'Update' : 'Launch'}</button>
            </form>
          </div>
        ...
        <div className="grid gap-4">
          {pendingReviews.map(r => (
            <div key={r.id} className="bg-slate-900 border border-slate-800 p-6 rounded-[2rem] flex justify-between">
              <div><p className="font-bold">"{r.text}"</p><p className="text-slate-500 text-xs">{r.casinoName} — {r.email}</p></div>
              <div className="flex gap-2"><button onClick={() => approveReview(r)} className="bg-emerald-600 px-4 py-2 rounded-xl text-xs font-black">APPROVE</button><button onClick={() => deleteReview(r)} className="bg-rose-900/50 px-4 py-2 rounded-xl text-xs font-black">DELETE</button></div>
            </div>
          ))}
        </div>
      )}
      ...
      <nav className="sticky top-0 z-50 bg-[#050608]/80 backdrop-blur-xl border-b border-slate-800/50 h-20 flex items-center px-6">
        <div className="max-w-7xl mx-auto w-full flex justify-between items-center">
          <div onClick={() => setView('public')} className="flex items-center gap-3 cursor-pointer group">
            ...
            <span className="font-black text-2xl tracking-tighter uppercase italic">CASINO<span className="text-indigo-500">PRO</span></span>
          </div>
          <div className="flex gap-3">
            {(!user || user.isAnonymous) ? (
              <button onClick={() => setView('auth')} className="bg-white text-black px-6 py-2 rounded-xl text-xs font-black uppercase">Login</button>
            ) : (
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => signOut(auth)} className="text-slate-400 hover:text-rose-400 transition-colors"><LogOut size={20} /></button>
              </div>
            )}
          </div>
        </div>
          ...
          <>
            ...
            {view === 'auth' && (
              <div className="max-w-md mx-auto mt-20 p-10 bg-slate-900 border border-slate-800 rounded-[3rem]">
                <h2 className="text-2xl font-black text-center mb-6">{isSignUp ? 'Join Elite' : 'Account Login'}</h2>
                <form onSubmit={handleAuth} className="space-y-4">
                  <input className="w-full bg-slate-800 rounded-xl p-4" placeholder="Email" value={authEmail} onChange={e=>setAuthEmail(e.target.value)} />
                  <input className="w-full bg-slate-800 rounded-xl p-4" type="password" placeholder="Password" value={authPass} onChange={e=>setAuthPass(e.target.value)} />
                  <button className="w-full bg-indigo-600 py-4 rounded-xl font-black">{isSignUp ? 'SIGN UP' : 'LOGIN'}</button>
                </form>
                <button onClick={()=>setIsSignUp(!isSignUp)} className="w-full mt-6 text-sm font-bold text-slate-500 hover:text-white">{isSignUp ? 'Already have an account? Log In' : "Don't have an account? Join Now"}</button>
              </div>
            )}
          </>
        )}
      </main>
      {showNotification && <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-indigo-600 px-6 py-3 rounded-2xl font-bold shadow-2xl z-[100]">{showNotification}</div>}
    </div>
  );
}
