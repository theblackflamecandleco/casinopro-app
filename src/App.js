...
import { 
  ...
  Underline, Type, Eraser, Minus, Check
} from 'lucide-react';
...
export default function App() {
  ...
  const [casinos, setCasinos] = useState(() => {
    const saved = localStorage.getItem('cp_final_perfect_cache');
  ...
  useEffect(() => {
    ...
    return onSnapshot(collection(db, 'casinos'), (snap) => {
      ...
      setCasinos(sorted);
      localStorage.setItem('cp_final_perfect_cache', JSON.stringify(sorted));
      setLoading(false);
    }, () => setLoading(false));
  }, []);
      ...
      <nav className="sticky top-0 z-[100] bg-[#050608]/95 backdrop-blur-md border-b border-slate-800/60 h-20 flex items-center px-6">
        <div className="max-w-7xl mx-auto w-full flex justify-between items-center">
          <div onClick={() => { setView('public'); setSelectedCasino(null); }} className="flex items-center gap-3 cursor-pointer group">
            ...
            <span className="font-black text-2xl tracking-tighter uppercase italic text-white">CASINOPRO</span>
          </div>

          <div className="flex gap-4 items-center">
            {isAdmin && <button onClick={() => setView('admin')} className="bg-indigo-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg">Admin</button>}
            <button onClick={() => (user && !user.isAnonymous) ? signOut(auth).then(()=>window.location.reload()) : setView('auth')} className="bg-white text-black px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl">
          ...
          <>
            ...
            {view === 'public' && (
              <div className="animate-in fade-in duration-500 pt-10">
                <div className="text-center py-10">
                  ...
                  <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.5em]">Direct Cloud Database Sync</p>
                </div>
                <div className="grid gap-4">
                  {casinos.map(c => (
                    <div key={c.id} onClick={() => { setSelectedCasino(c); setView('details'); window.scrollTo(0,0); }} className="bg-slate-900/40 border border-slate-800 rounded-[1.5rem] p-4 flex items-center gap-4 hover:border-indigo-500/50 transition-all active:scale-95 cursor-pointer shadow-xl">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-xl font-bold text-white truncate uppercase italic">{c.name}</h3>
            ...
            {view === 'details' && selectedCasino && (
              <div className="pt-10 space-y-10 animate-in fade-in">
                <button onClick={() => setView('public')} className="text-indigo-400 font-bold mb-6 flex items-center gap-2 uppercase text-[10px] tracking-widest transition-colors hover:text-white"><ArrowLeft size={16}/> Back</button>
                <div className="bg-slate-900 border border-slate-800 rounded-[3rem] overflow-hidden shadow-2xl">
                  ...
                  <div className="px-10 pb-10 -mt-16 relative z-10">
                    ...
                    <div className="border-t border-slate-800 pt-8 prose prose-invert max-w-none text-slate-300 text-sm italic leading-relaxed" dangerouslySetInnerHTML={{ __html: selectedCasino.description }} />
                  </div>
                </div>
                <div className="space-y-6 pb-20">
                  <h2 className="text-2xl font-black text-white uppercase italic flex items-center gap-2 tracking-tight"><MessageSquare size={20} className="text-indigo-500" /> Feedback</h2>
                  {(!user || user.isAnonymous) ? (
                    <div className="bg-slate-900/50 border border-slate-800 p-10 rounded-[2rem] text-center"><button onClick={() => setView('auth')} className="bg-white text-black px-10 py-3 rounded-2xl font-black text-[10px] uppercase shadow-xl">Sign In to Post</button></div>
                  ) : (
            ...
            {view === 'admin' && isAdmin && (
              <div className="pt-10 space-y-10 animate-in zoom-in-95">
                <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-5 rounded-[2rem] shadow-xl">
                  ...
                  <button onClick={() => setView('public')} className="text-[10px] font-black text-indigo-400 uppercase tracking-widest flex items-center gap-2"><Eye size={16}/> Site</button>
                </div>
                {adminTab === 'casinos' ? (
                  <div className="grid lg:grid-cols-3 gap-8 pb-40">
                    <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-8 rounded-[3.5rem] shadow-2xl">
                      <h2 className="text-xl font-black text-white mb-8 italic uppercase tracking-tighter italic border-l-4 border-indigo-600 pl-4">{isEditing ? 'Modify Review' : 'New Listing'}</h2>
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                          ...
                          <div className="relative">
                            ...
                            <label htmlFor="logo-up" className="flex items-center justify-center gap-2 w-full bg-slate-800 p-5 rounded-2xl text-slate-400 cursor-pointer border border-dashed border-slate-700">
                              {formData.logo ? <Check size={18} className="text-emerald-500" /> : <ImageIcon size={18} />}
                        ...
                        <div className="space-y-2">
                          <div className="border border-slate-800 rounded-3xl overflow-hidden bg-black/40 shadow-inner">
                            <div className="bg-slate-800 p-2 flex gap-1 border-b border-slate-700 overflow-x-auto no-scrollbar">
                              ...
                              <button type="button" onClick={() => exec('italic')} className="p-2.5 hover:bg-indigo-600 rounded-lg text-white italic font-serif">I</button>
                              ...
                              <div className="w-[1px] bg-slate-700 mx-1"></div>
                              <button type="button" onClick={() => exec('foreColor', '#10b981')} className="p-2.5 hover:bg-emerald-600 rounded-lg text-emerald-400 font-black text-[10px]">Green</button>
                              <button type="button" onClick={() => exec('foreColor', '#f43f5e')} className="p-2.5 hover:bg-rose-600 rounded-lg text-rose-400 font-black text-[10px]">Red</button>
                            ...
                            <div ref={editorRef} contentEditable className="p-8 min-h-[350px] text-white outline-none focus:ring-1 focus:ring-indigo-500 bg-slate-900/50 prose prose-invert max-w-none text-base italic leading-relaxed" onBlur={e => setFormData(p => ({ ...p, description: e.target.innerHTML }))} />
                          </div>
                        </div>

                        <div className="grid md:grid-cols-2 gap-6">
                          <input className="bg-slate-800 rounded-2xl p-5 text-white outline-none font-bold italic text-sm" placeholder="Bonus" value={formData.bonus} onChange={e => setFormData({...formData, bonus: e.target.value})} required />
                          <input className="bg-slate-800 rounded-2xl p-5 text-indigo-400 font-mono text-xs outline-none" placeholder="URL" value={formData.link} onChange={e => setFormData({...formData, link: e.target.value})} required />
                        </div>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-xl h-fit">
                      ...
                      <div className="divide-y divide-slate-800/50 max-h-[500px] overflow-y-auto no-scrollbar">
                        {casinos.map(c => (
                  ...
                  <div className="grid gap-4 max-w-4xl mx-auto pb-40">
                    ...
                    {pendingReviews.map(r => (
                      <div key={r.id} className="bg-slate-900 border border-slate-800 p-10 rounded-[3rem] flex flex-col md:flex-row justify-between gap-10 shadow-2xl animate-in zoom-in-95">
                        <div className="flex-1 space-y-4"><p className="text-white text-lg font-medium leading-relaxed italic uppercase tracking-tight">"{r.text}"</p><p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">— {r.email} ({r.casinoName})</p></div>
                        <div className="flex items-center gap-4 shrink-0"><button onClick={async () => await updateDoc(doc(db, 'casinos', r.casinoId, 'user_reviews', r.id), { isApproved: true })} className="bg-emerald-600 hover:bg-emerald-500 px-8 py-4 rounded-2xl font-black text-xs text-white uppercase shadow-lg">Approve</button><button onClick={async () => await deleteDoc(doc(db, 'casinos', r.casinoId, 'user_reviews', r.id))} className="bg-slate-800 hover:bg-rose-900/50 px-6 py-4 rounded-2xl font-black text-xs text-white border border-slate-700 uppercase">Reject</button></div>
                      </div>
                    ))}
                    {pendingReviews.length === 0 && <div className="p-20 text-center bg-slate-900/50 border border-slate-800 rounded-[3.5rem] italic text-slate-700 font-black uppercase text-xs tracking-[0.3em]">No Pending Items</div>}
                  </div>
                )}
              </div>
            )}
            ...
            {view === 'auth' && (
              <div className="max-w-md mx-auto mt-20 p-12 bg-slate-900 border border-slate-800 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95">
                ...
                <form onSubmit={async e => { e.preventDefault(); try { if (isSignUp) await createUserWithEmailAndPassword(auth, authEmail, authPass); else await signInWithEmailAndPassword(auth, authEmail, authPass); setView('public'); } catch (err) { alert(err.message); } }} className="space-y-4">
                  ...
                  <button className="w-full bg-indigo-600 py-5 rounded-3xl font-black text-white uppercase tracking-[0.2em] shadow-xl">CONTINUE</button>
                </form>
