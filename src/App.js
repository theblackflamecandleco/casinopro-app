...
export default function App() {
  ...
  const [casinos, setCasinos] = useState(() => {
    const saved = localStorage.getItem('cp_final_cache_v2');
  ...
  useEffect(() => {
    ...
    return onSnapshot(collection(db, 'casinos'), (snap) => {
      ...
      setCasinos(sorted);
      localStorage.setItem('cp_final_cache_v2', JSON.stringify(sorted));
      setLoading(false);
    }, () => setLoading(false));
  }, []);
      ...
      <nav className="sticky top-0 z-[100] bg-[#050608]/95 backdrop-blur-md border-b border-slate-800/50 h-16 flex items-center px-6">
        <div className="max-w-7xl mx-auto w-full flex justify-between items-center">
          <div onClick={() => { setView('public'); setSelectedCasino(null); }} className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="font-black text-xl tracking-tighter uppercase italic">CASINOPRO</span>
          </div>
          <div className="flex gap-4">
            {isAdmin && <button onClick={() => setView('admin')} className="text-[10px] font-black text-indigo-400 border border-indigo-500/30 px-4 py-2 rounded-xl bg-indigo-500/5 uppercase tracking-widest">Admin</button>}
          ...
          <>
            {view === 'public' && (
              <div className="max-w-5xl mx-auto p-4 pt-10 space-y-6 animate-in fade-in">
                <div className="text-center py-10">
                  <h1 className="text-5xl font-black mb-2 tracking-tighter italic uppercase">ELITE RANKINGS</h1>
                  <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em]">Verified Live Feed</p>
                </div>
                <div className="grid gap-4">
                  {casinos.map(c => (
                    <div key={c.id} onClick={() => { setSelectedCasino(c); setView('details'); window.scrollTo(0,0); }} className="bg-slate-900/40 border border-slate-800 rounded-[1.5rem] p-4 flex items-center gap-4 hover:border-indigo-500/50 transition-all active:scale-95 cursor-pointer">
                      <div className="w-16 h-16 rounded-2xl overflow-hidden bg-slate-800 shrink-0 border border-slate-700">
                        {c.logo ? <img src={c.logo} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full flex items-center justify-center text-xl font-black bg-indigo-600">{c.name.charAt(0)}</div>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="text-lg font-bold text-white truncate">{c.name}</h3>
                        <div className="flex items-center gap-1.5 mt-1"><Star size={10} className="text-amber-400 fill-amber-400" /><span className="text-[10px] text-slate-500 uppercase font-black">{c.rating} Expert Rating</span></div>
                      </div>
              ...
              <div className="max-w-4xl mx-auto p-4 pt-10 space-y-10 animate-in fade-in">
                  ...
                  <div className="px-10 pb-10 -mt-16 relative z-10">
                    <div className="flex flex-col md:flex-row items-end gap-6 mb-8">
                      <div className="w-32 h-32 rounded-3xl overflow-hidden bg-slate-800 border-4 border-slate-900">
                        {selectedCasino.logo ? <img src={selectedCasino.logo} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full bg-indigo-600 flex items-center justify-center text-4xl font-black text-white italic">{selectedCasino.name.charAt(0)}</div>}
                      </div>
                    ...
                    <div className="border-t border-slate-800 pt-8 prose prose-invert max-w-none text-slate-300 text-sm italic" dangerouslySetInnerHTML={{ __html: selectedCasino.description }} />
                  </div>
                </div>
                <div className="space-y-6 pb-20">
                  ...
                  {(!user || user.isAnonymous) ? (
                    <div className="bg-slate-900/50 border border-slate-800 p-10 rounded-[2rem] text-center"><button onClick={() => setView('auth')} className="bg-white text-black px-10 py-3 rounded-2xl font-black text-[10px] uppercase shadow-xl">Login to Share Experience</button></div>
                  ) : (
                    <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2rem] shadow-xl">
                      <form onSubmit={async (e) => {
                        ...
                        <textarea required placeholder="Your experience..." className="w-full bg-slate-800 border-none rounded-xl p-4 text-white h-24 outline-none focus:ring-2 focus:ring-indigo-500 text-sm italic" value={newReview.text} onChange={e => setNewReview({...newReview, text: e.target.value})} />
              ...
              <div className="max-w-6xl mx-auto p-4 pt-10 space-y-10 animate-in zoom-in-95">
                <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-5 rounded-[2rem] shadow-xl">
                  <div className="flex gap-3">
                    ...
                    <button onClick={() => setAdminTab('moderation')} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all ${adminTab === 'moderation' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 bg-slate-800'}`}>Moderation ({pendingReviews.length})</button>
                  </div>
                ...
                {adminTab === 'casinos' ? (
                  <div className="grid lg:grid-cols-3 gap-8 pb-40">
                    <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-10 rounded-[3.5rem] shadow-2xl">
                      <h2 className="text-xl font-black text-white mb-8 italic uppercase tracking-tighter flex items-center gap-2">{isEditing ? 'Edit Partner' : 'New Listing'}</h2>
                      <form onSubmit={handleSave} className="space-y-6">
                        <div className="grid md:grid-cols-2 gap-6">
                          ...
                          <div className="relative">
                            ...
                            <label htmlFor="logo-up" className="flex items-center justify-center gap-2 w-full bg-slate-800 p-5 rounded-2xl text-slate-400 cursor-pointer border border-dashed border-slate-700">
                              {formData.logo ? <CheckCircle2 size={18} className="text-emerald-500" /> : <ImageIcon size={18} />}
                              <span className="font-bold text-[10px] uppercase tracking-widest">{formData.logo ? 'ID Set' : 'Upload Icon'}</span>
                            </label>
                          </div>
                        </div>
                        <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 italic">Visual Editor</label>
                          <div className="border border-slate-800 rounded-3xl overflow-hidden bg-black/40">
                            <div className="bg-slate-800 p-2 flex gap-1 border-b border-slate-700 overflow-x-auto">
                              <button type="button" onClick={() => exec('bold')} className="p-2 hover:bg-slate-700 rounded text-white font-bold">B</button>
                              <button type="button" onClick={() => exec('italic')} className="p-2 hover:bg-slate-700 rounded text-white italic">I</button>
                              <button type="button" onClick={() => exec('formatBlock', 'H2')} className="p-2 hover:bg-slate-700 rounded text-white font-black text-xs">H2</button>
                              <button type="button" onClick={() => exec('insertUnorderedList')} className="p-2 hover:bg-slate-700 rounded text-white font-black text-xs">List</button>
                            </div>
                            <div ref={editorRef} contentEditable className="p-8 min-h-[300px] text-white outline-none focus:ring-1 focus:ring-indigo-500 bg-slate-900/50 prose prose-invert max-w-none text-sm italic" onBlur={e => setFormData(p => ({ ...p, description: e.target.innerHTML }))} />
                          </div>
                        </div>
                        <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <button className="w-full bg-indigo-600 py-6 rounded-3xl font-black text-white uppercase tracking-[0.2em] shadow-xl hover:bg-indigo-500 transition-all">Launch Listing</button>
                      </form>
                    </div>
                    <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] overflow-hidden shadow-2xl h-fit">
                      <div className="p-6 border-b border-slate-800 font-black text-[10px] text-slate-500 uppercase tracking-widest bg-slate-800/20 italic text-center">Active Directory</div>
                      <div className="divide-y divide-slate-800/50">
                        {casinos.map(c => (
                          <div key={c.id} className="p-5 flex items-center justify-between hover:bg-slate-800/30">
                            <div className="flex items-center gap-3 min-w-0"><div className="w-10 h-10 rounded-lg bg-slate-800 overflow-hidden shrink-0 border border-slate-700">{c.logo && <img src={c.logo} className="w-full h-full object-cover" />}</div><span className="font-bold text-sm text-white italic truncate">{c.name}</span></div>
                            <div className="flex gap-1 shrink-0"><button onClick={() => { setIsEditing(c.id); setFormData(c); if(editorRef.current) editorRef.current.innerHTML = c.description; window.scrollTo({top: 0, behavior: 'smooth'}); }} className="p-2 text-slate-500 hover:text-indigo-400"><Edit3 size={18}/></button><button onClick={async () => { if(window.confirm('Delete?')) await deleteDoc(doc(db, 'casinos', c.id)); }} className="p-2 text-slate-500 hover:text-rose-400"><Trash2 size={18}/></button></div>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="grid gap-4 max-w-4xl mx-auto pb-40">
                    <h2 className="text-xl font-black text-white mb-8 uppercase tracking-tighter italic border-l-4 border-indigo-600 pl-4">Community Moderation</h2>
                    {pendingReviews.map(r => (
                      <div key={r.id} className="bg-slate-900 border border-slate-800 p-10 rounded-[3rem] flex flex-col md:flex-row justify-between gap-10 shadow-2xl animate-in zoom-in-95">
                        <div className="flex-1 space-y-4">
                          <p className="text-white text-lg font-medium leading-relaxed italic uppercase tracking-tight">"{r.text}"</p>
                          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">— {r.email} ({r.casinoName})</p>
                        </div>
                        <div className="flex items-center gap-4 shrink-0">
                          <button onClick={async () => await updateDoc(doc(db, 'casinos', r.casinoId, 'user_reviews', r.id), { isApproved: true })} className="bg-emerald-600 hover:bg-emerald-500 px-8 py-4 rounded-2xl font-black text-xs text-white uppercase shadow-lg shadow-emerald-600/20">Approve</button>
                          <button onClick={async () => await deleteDoc(doc(db, 'casinos', r.casinoId, 'user_reviews', r.id))} className="bg-slate-800 hover:bg-rose-900/50 px-6 py-4 rounded-2xl font-black text-xs text-white border border-slate-700 uppercase">Reject</button>
                        </div>
                      </div>
                    ))}
                    {pendingReviews.length === 0 && <div className="p-20 text-center bg-slate-900/50 border border-slate-800 rounded-[3.5rem] italic text-slate-700 font-black uppercase text-xs tracking-[0.3em]">No Pending Reviews</div>}
                  </div>
                )}
              </div>
            )}
            ...
            {view === 'auth' && (
              <div className="max-w-md mx-auto mt-20 p-12 bg-slate-900 border border-slate-800 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95">
                <div className="text-center mb-10"><div className="w-20 h-20 bg-indigo-600 rounded-3xl mx-auto flex items-center justify-center shadow-2xl shadow-indigo-600/30 mb-6 font-black text-white text-3xl">CP</div><h2 className="text-2xl font-black text-white uppercase tracking-tighter italic">{isSignUp ? 'New Profile' : 'Member Login'}</h2></div>
                <form onSubmit={async e => { e.preventDefault(); try { if (isSignUp) await createUserWithEmailAndPassword(auth, authEmail, authPass); else await signInWithEmailAndPassword(auth, authEmail, authPass); setView('public'); } catch (err) { alert(err.message); } }} className="space-y-4">
                  ...
                  <button className="w-full bg-indigo-600 py-5 rounded-3xl font-black text-white uppercase tracking-[0.2em] shadow-xl">Authenticate</button>
                </form>
                <button onClick={()=>setIsSignUp(!isSignUp)} className="w-full mt-8 text-[10px] font-black text-slate-500 uppercase tracking-widest">{isSignUp ? 'Login' : 'Join Now'}</button>
              </div>
            )}
          </>
        )}
