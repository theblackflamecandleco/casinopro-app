...
import { 
  ...
  Bold, Italic, List, Heading1, Heading2, Eye, ShieldAlert, Check
} from 'lucide-react';
...
export default function App() {
  ...
  const [casinos, setCasinos] = useState(() => {
    const saved = localStorage.getItem('cp_cache_master_save');
  ...
  useEffect(() => {
    ...
    const unsubData = onSnapshot(q, (snapshot) => {
      ...
      setCasinos(sorted);
      localStorage.setItem('cp_cache_master_save', JSON.stringify(sorted));
      setLoading(false);
    }, () => setLoading(false));
      ...
      <main>
        {loading && casinos.length === 0 ? (
          ...
          <>
            ...
            {view === 'public' && (
              <div className="max-w-5xl mx-auto space-y-6 p-4 pt-10 animate-in fade-in">
                <div className="text-center py-10">
                  ...
                  <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.4em]">Official 2024 Vetted Listings</p>
                </div>
                <div className="grid gap-4">
                  {casinos.map((c) => (
                    <div key={c.id} onClick={() => { setSelectedCasino(c); setView('details'); window.scrollTo(0,0); }} className="group cursor-pointer bg-slate-900/40 border border-slate-800 rounded-[1.5rem] p-4 flex items-center gap-4 hover:border-indigo-500/50 transition-all active:scale-[0.98]">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-1">
                          ...
                          <span className="text-[10px] font-black text-slate-500 uppercase">{c.rating} Rating</span>
                        </div>
                      </div>
            ...
            {view === 'details' && selectedCasino && (
              <div className="max-w-4xl mx-auto p-4 pt-10 space-y-10 animate-in fade-in duration-300">
                   ...
                   <div className="px-10 pb-10 -mt-16 relative z-10">
                     <div className="flex flex-col md:flex-row items-end gap-6 mb-8">
                       ...
                       <div className="flex-1 pb-1">
                         <h1 className="text-4xl font-black text-white italic uppercase tracking-tight">{selectedCasino.name}</h1>
                       </div>
                ...
                <div className="space-y-6 pb-20">
                  <h2 className="text-2xl font-black text-white uppercase flex items-center gap-2 italic"><MessageSquare size={20} className="text-indigo-500" /> Community Feedback</h2>
                  {(!user || user.isAnonymous) ? (
                    <div className="bg-slate-900/50 border border-slate-800 p-10 rounded-[3rem] text-center">
                      <button onClick={() => setView('auth')} className="bg-white text-black px-10 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest">Sign In to Review</button>
                    </div>
                  ) : (
                    <div className="bg-slate-900 border border-slate-800 p-8 rounded-[2.5rem] shadow-xl">
                      <form onSubmit={async (e) => {
                        ...
                        <textarea required placeholder="How was your experience?" className="w-full bg-slate-800 border-none rounded-2xl p-6 text-white h-24 outline-none focus:ring-2 focus:ring-indigo-500 text-sm italic" value={newReview.text} onChange={(e) => setNewReview({...newReview, text: e.target.value})} />
                        <button type="submit" className="bg-indigo-600 px-8 py-3 rounded-xl font-black text-[10px] text-white uppercase tracking-widest">Submit Review</button>
                      </form>
                    </div>
                  )}
            ...
            {/* VIEW: ADMIN PANEL */}
            {view === 'admin' && isAdmin && (
              <div className="max-w-6xl mx-auto space-y-8 p-4 pt-10 animate-in zoom-in-95">
                <div className="flex justify-between items-center bg-slate-900 border border-slate-800 p-5 rounded-[2.5rem] shadow-xl">
                  <div className="flex gap-3">
                    ...
                    <button onClick={() => setAdminTab('moderation')} className={`px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${adminTab === 'moderation' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-500 bg-slate-800'}`}>Moderation ({pendingReviews.length})</button>
                  </div>
                  <button onClick={() => setView('public')} className="text-xs font-black text-indigo-400 hover:text-white flex items-center gap-2 uppercase tracking-widest"><Eye size={16}/> Live Site</button>
                </div>
                  ...
                  <div className="grid lg:grid-cols-3 gap-8 pb-40">
                    <div className="lg:col-span-2 bg-slate-900 border border-slate-800 p-10 rounded-[3rem] shadow-2xl">
                      <h2 className="text-xl font-black text-white mb-8 italic uppercase tracking-tighter flex items-center gap-2">
                        ...
                        {isEditing ? 'Modify Listing' : 'Create Listing'}
                      </h2>
                      <form onSubmit={handleSaveCasino} className="space-y-6">
                        <div className="grid md:grid-cols-2 gap-6">
                          ...
                          <div className="relative">
                            ...
                            <label htmlFor="logo-up" className="flex items-center justify-center gap-2 w-full bg-slate-800 p-5 rounded-2xl text-slate-400 cursor-pointer hover:bg-slate-700 border border-dashed border-slate-700">
                              ...
                              <span className="font-bold text-[10px] uppercase tracking-widest">{formData.logo ? 'Logo Uploaded' : 'Upload Logo'}</span>
                            </label>
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="border border-slate-800 rounded-3xl overflow-hidden bg-black/40">
                            <div className="bg-slate-800 p-2 flex gap-1 border-b border-slate-700 overflow-x-auto">
                              ...
                              <button type="button" onClick={() => execCmd('italic')} className="p-2 hover:bg-slate-700 rounded text-white italic">I</button>
                            ...
                            <div 
                              ref={editorRef}
                              contentEditable
                              className="p-8 min-h-[350px] text-white outline-none focus:ring-1 focus:ring-indigo-500 bg-slate-900/50 prose prose-invert max-w-none text-sm leading-relaxed"
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                          <input className="bg-slate-800 rounded-2xl p-5 text-white outline-n
